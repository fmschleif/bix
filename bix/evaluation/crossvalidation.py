...
class CrossValidation(Study):
    ...
    def __init__(self, clfs, parallel = -1,streams=None, test_size=1, path="study", param_path="search", max_samples=1000000):
        ...
        if max_samples < 500:
            raise ValueError("Max Samples must be at least 500!")
    ...
    def parameter_grid_search(self,grid):

        for clf in self.clfs:
            parameter_grid,clf_objects = self.expand_grid(grid,clf)
            self.clfs = clf_objects
            self.test()
            self.save_summary()
            print("done")
    ...
    def expand_grid(self,grid,clf):
        parameter_grid = list(itertools.product(*list(grid.values())))
        clf_objects = [copy.deepcopy(clf) for i in parameter_grid]

        names = list(grid.keys())
        for i,clf in enumerate(clf_objects):
            for j,name in enumerate(names):
                param = parameter_grid[i][j]
                clf.__dict__[name] = int(param) if param.dtype == 'int32' else param

        return parameter_grid,clf_objects

    def test(self):
        start = time.time()
        grid = self.create_grid(self.clfs, self.streams)
        self.result.extend(Parallel(n_jobs=self.parallel,max_nbytes=None )
